...
async function callMailApi(payload) {
  ...
  try {
    ...
    // CORS 차단도 여기로 떨어진다(브라우저가 이유를 안 알려준다).
    // 가장 흔한 원인은 doPost가 아직 배포되지 않아 Apps Script가 오류 페이지를 돌려주는 경우다.
    return {
      ok: false,
      code: "network",
      message:
        "발송 서버에 연결하지 못했습니다. ① apps-script/Code.gs의 doPost를 붙여넣고 '새 버전'으로 다시 배포했는지 ② 액세스 권한이 '모든 사용자'인지 ③ 실행 계정이 '나'인지 확인해주세요. (배포 전에는 브라우저가 CORS로 막습니다)",
    };
  }
