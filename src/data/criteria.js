...
// 2차 항목을 데스크 추정으로 채우면서(scripts/score-phase2.mjs) 총점 상한이 29.6 → 41.4로
// 올라갔다. 임계값을 그대로 두면 A가 9곳에서 42곳으로 불어나 변별력이 사라지므로, 위와 같은
// 방식(실제 점수 분포의 상위 11%/43%/39%/7% 지점)으로 다시 잡았다.
// 추정이 아닌 실제 자료로 2차를 채우면 이 값도 다시 조정해야 한다.
export const GRADES = {
  A: { min: 33, label: "A 등급", desc: "1차 조사 우수 — 2차 검증 즉시 착수", color: "teal" },
  B: { min: 22, label: "B 등급", desc: "검토 대상 — 2차 확인 필요", color: "blue" },
  C: { min: 13, label: "C 등급", desc: "보류", color: "amber" },
